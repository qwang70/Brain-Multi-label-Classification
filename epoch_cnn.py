...
modelcheckpoint = ModelCheckpoint(saved_model, monitor='val_loss', save_best_only=True, mode='auto', period=1)
callbacks_list = [histories, earlystop, modelcheckpoint]
# wait for loss < 0.5 wwwwwww
history = model.fit(train_X, augmented_Y,
                    ...
                    verbose=2)
"""
history = model.fit(train_X[:4371], augmented_Y[:4371],
                    epochs=1,
                    batch_size=50,
                    shuffle=False,
                    verbose=2,
                    validation_data=(train_X[4372:], augmented_Y[4372:]),
                    )
"""
# list all data in history
print("losses", histories.losses)
print("aucs", histories.aucs)
print("fantastic val_loss! {}".format(history.history['val_loss'][-7:]))
...
np.save("epoch_label_result.npy", pred_labels)
